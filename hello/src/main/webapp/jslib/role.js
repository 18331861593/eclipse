...
function editRole(){
	...
	d.dialog({
		...
		width:600,
		modal : true,
		href : "roleEdit.action?id="+selRow.id,
		onClose:function(){$(this).dialog("destroy");},
		onLoad:function(){
			//让dialog垂直居中
			$(this).dialog("vcenter");
			//发送一一步请求，获取要编辑的数据
			$.post("roleToEdit.action",{"id":selRow.id},function(data){
...
function deleteRole(){
	//获取选中的行
	var selRows = $("#roleList").datagrid("getSelected");
	//如果没有选中的行，弹框提示
	if(selRows == null){
	...
	$.messager.confirm("提示","确定要删除选中的数据吗",function(r){
		if(r){
			//发送异步请求，批量删除数据，
			$.post("roleDelete.action",{id : selRows.id},function(data){
				//删除成功，重新加载datagrid
				$("#roleList").datagrid("reload");
			});
		}
