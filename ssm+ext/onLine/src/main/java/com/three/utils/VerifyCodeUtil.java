...
//	private String verifyCode;
	...
	public static void init(HttpServletRequest request, HttpServletResponse response){
		  ...
		  //BufferedImage类是具有缓冲区的Image类,Image类是用于描述图像信息的类
	        ...
	        System.out.println("生成的验证码是 ：" + randomString);
	        session.removeAttribute(Constants.RANDOMCODEKEY);
	        session.setAttribute(Constants.RANDOMCODEKEY, randomString);
    ...
    public static String getCode(HttpServletRequest request){
    	...
    	if(session.getAttribute(Constants.RANDOMCODEKEY) == null)
    		return Constants.RANDOMCODEKEY;
    	else
    		return (String)session.getAttribute(Constants.RANDOMCODEKEY);
    }
